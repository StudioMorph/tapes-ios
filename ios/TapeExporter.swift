...
enum TapeExporter {
    static func export(tape: Tape, completion: @escaping (URL?, String?) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            let seq: [TransitionStyle] = (tape.transition == .randomise)
                ? TransitionPicker.sequenceForTape(tapeId: tape.id, boundaries: boundaries)
                : Array(repeating: tape.transition, count: boundaries)
            let dur = (tape.transition == .randomise) ? TransitionPicker.clampedDuration(tape.transitionDuration) : tape.transitionDuration
