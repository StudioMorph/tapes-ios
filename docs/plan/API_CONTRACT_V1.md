...
## R2 Upload Flow (Batch — current)

```
iOS                              Workers API                    R2
 |                                    |                          |
 |  POST /tapes/:id/prepare-upload   |                          |
 |  { clips: [{clip_id, type, ...}], |                          |
 |    batch_type, mode }              |                          |
 |  ─────────────────────────────────>|                          |
 |                                    |  Create N clip records   |
 |                                    |  Generate N presigned    |
 |                                    |  URL sets (2h expiry)    |
 |  { batch_id, clips: [{upload_url, |                          |
 |    thumbnail_upload_url, ...}×N] } |                          |
 |  <─────────────────────────────────|                          |
 |                                    |                          |
 |  Submit ALL uploads at once        |                          |
 |  (N media + N thumbs + K movies)  |                          |
 |  ════════════════════════════════════════════════════════>    |
 |                                    |                          |
 |  ── App can be killed. OS daemon continues all transfers ──  |
 |                                    |                          |
 |  POST /tapes/:id/confirm-batch    |                          |
 |  { clips: [{clip_id, cloud_url,  |                          |
 |    thumbnail_url, ...}×N] }       |                          |
 |  ─────────────────────────────────>|  Confirm all clips       |
...
Creates all clip D1 records and returns presigned R2 URLs in a single request.
...
```json
{
  ...
  "batch_type": "invite|update",
  "mode": "view_only|collaborative"
}
```
  ...
  "clips": [
    {
      ...
      "thumbnail_upload_url": "https://...presigned...",
      "live_photo_movie_upload_url": null,
      "upload_url_expires_at": "2026-04-27T03:00:00Z",
      "order_index": 1
    }
  ],
  "expected_count": 1
...
**Notes:**
- Max 200 clips per request. Returns 422 if exceeded.
- Presigned URLs use a 2-hour expiry.
- Idempotent per clip_id: re-preparing an existing clip returns fresh URLs.
- Also creates the upload batch record (replaces `POST /upload-batch` for this flow).

### `POST /tapes/:id/confirm-batch`

Confirms all uploaded clips in a single request.
...
```json
{
  "clips": [
    {
      "clip_id": "uuid",
      "cloud_url": "https://...base-url...",
      "thumbnail_url": "https://...base-url...",
...
- Partial confirms supported: if only 60 of 66 clips uploaded, confirm those 60.
- Sends push notification when `batch_completed` is true.

## R2 Upload Flow (Legacy — per-clip)

The old per-clip flow (`POST /tapes/:id/clips` + `POST /clips/:id/uploaded`) remains fully functional for backward compatibility and single-clip operations (e.g. contribute flow).
